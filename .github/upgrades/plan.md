...
## Overview
This plan outlines the steps to upgrade the AsyncSpark solution from .NET 9 to .NET 10. The solution contains 6 projects with various dependencies and NuGet packages that need to be updated.

## Projects to Upgrade
1. **AsyncSpark.csproj** - Core class library (3 dependants)
2. **AsyncSpark.Tests.csproj** - Test project
3. **AsyncSpark.Console.csproj** - Console application
4. **AsyncSpark.Web.csproj** - ASP.NET Core Razor Pages application
...
2. Run test suite (AsyncSpark.Tests)
